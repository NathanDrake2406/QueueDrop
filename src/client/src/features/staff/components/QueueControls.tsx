      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
        ...
        <div className="flex gap-8">
          <div className="text-center">
            ...
            <div className="text-xs text-slate-500 uppercase tracking-wide">Waiting</div>
          </div>
          <div className="text-center">
            <div className="text-4xl font-bold text-amber-500">{queueInfo.calledCount}</div>
            <div className="text-xs text-slate-500 uppercase tracking-wide">Called</div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        {showAllControls && onCallNext && (
          <button
            onClick={onCallNext}
            disabled={!hasWaiting}
            className="flex-1 sm:flex-none px-8 py-4 bg-white text-slate-900 rounded-2xl font-semibold hover:bg-slate-100 disabled:bg-slate-800 disabled:text-slate-600 disabled:cursor-not-allowed transition-colors"
          >
            Call Next
          </button>
        )}
        {!showAllControls && (
          <div className="flex-1 sm:flex-none px-8 py-4 bg-slate-800 text-slate-500 rounded-2xl font-medium text-center">
        ...
        {showAllControls && onOpenQRCode && (
          <button
            ...
            className="px-6 py-4 bg-teal-600 text-white rounded-2xl font-medium hover:bg-teal-500 transition-colors flex items-center gap-2"
          >
        ...
        {showAllControls && onOpenSettings && (
          <button
            ...
            className="px-6 py-4 bg-slate-800 text-slate-300 rounded-2xl font-medium hover:bg-slate-700 transition-colors"
          >
