          ...
          <div className="flex-1 min-w-0">
            ...
            {(queueName || customer.partySize || customer.notes) && (
              <div className="mt-2 flex flex-wrap gap-2">
                {queueName && (
                  <span className="inline-flex items-center px-2 py-0.5 bg-teal-500/20 rounded-lg text-xs text-teal-400 font-medium">
                    {queueName}
                  </span>
                )}
                {customer.partySize && (
                  <span className="inline-flex items-center px-2 py-0.5 bg-slate-800 rounded-lg text-xs text-slate-400">
                    Party of {customer.partySize}
                  </span>
                )}
                {customer.notes && (
                  <span className="inline-flex items-center px-2 py-0.5 bg-slate-800 rounded-lg text-xs text-slate-400 italic max-w-[200px] truncate">
                    {customer.notes}
                  </span>
                )}
              </div>
        ...
        <div className="flex gap-2 ml-4">
          ...
          {isCalled && onMarkNoShow && (
            <button
              ...
              className="px-4 py-2 bg-slate-700 text-slate-300 text-sm font-medium rounded-xl hover:bg-slate-600 transition-colors"
            >
              No-show
            </button>
          )}
