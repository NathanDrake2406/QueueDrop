...
export function UserMenu() {
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        {/* Avatar */}
        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-teal-500 to-teal-500 flex items-center justify-center text-white font-semibold text-sm">
          {avatarLetter}
        </div>
        {/* Email (hidden on mobile) */}
        <span className="hidden sm:block text-sm text-slate-300 max-w-[150px] truncate">
          {user.email}
        </span>
        {/* Chevron */}
        <svg
        ...
        <div className="absolute right-0 mt-2 w-64 bg-slate-900 border border-slate-800 rounded-xl shadow-xl py-2 z-50">
          ...
          {businesses.length > 1 && (
            <div className="py-2 border-b border-slate-800">
              <p className="px-4 py-1 text-xs text-slate-500 uppercase tracking-wide">Switch business</p>
          ...
          {businesses.length === 1 && (
            <div className="py-1 border-b border-slate-800">
              <button
                ...
                className="w-full px-4 py-2 text-left text-sm text-slate-300 hover:bg-slate-800 transition-colors flex items-center gap-2"
              >
